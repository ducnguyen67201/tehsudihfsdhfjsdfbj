...
function createRealisticDelegate(records: MockRecord[]) {
  return {
    deleteMany: vi.fn().mockImplementation(({ where }: { where: { deletedAt: { not: null; lt: Date } } }) => {
    ...
    count: vi.fn().mockImplementation(({ where }: { where: { deletedAt: { not: null; lt: Date } } }) => {
      const cutoff = where.deletedAt.lt;
      return Promise.resolve(records.filter((r) => r.deletedAt !== null && r.deletedAt < cutoff).length);
    }),
    findFirst: vi.fn().mockImplementation(({ where }: { where: { id: string; deletedAt: { not: null } } }) => {
      return Promise.resolve(records.find((r) => r.id === where.id && r.deletedAt !== null) ?? null);
    }),
...
function createMockClient(overrides: Record<string, ReturnType<typeof createRealisticDelegate>> = {}) {
  const models = [
    "user", "workspace", "workspaceMembership", "workspaceApiKey",
    "supportInstallation", "supportConversation", "supportDeliveryAttempt", "supportTicketLink",
  ];
...
describe("purge: retention window", () => {
  ...
  it("does not purge active records", async () => {
    const { purgeDeletedRecords } = await load();
    const records: MockRecord[] = [{ id: "w1", deletedAt: null }, { id: "w2", deletedAt: null }];
...
describe("purge: dependency ordering", () => {
  it("deletes children before parents", async () => {
    ...
    expect(callOrder.indexOf("supportTicketLink")).toBeLessThan(callOrder.indexOf("supportConversation"));
    expect(callOrder.indexOf("supportConversation")).toBeLessThan(callOrder.indexOf("supportInstallation"));
...
describe("hardDeleteById: safety", () => {
  it("deletes a soft-deleted record", async () => {
    ...
    const records: MockRecord[] = [{ id: "u1", deletedAt: YESTERDAY }, { id: "u2", deletedAt: null }];
    const client = createMockClient({ user: createRealisticDelegate(records) });
  ...
  it("refuses active records", async () => {
    ...
    await expect(hardDeleteById(client as never, "User", "u1")).rejects.toThrow("not found or not soft-deleted");
    expect(records).toHaveLength(1);
  });
  ...
  it("refuses nonexistent records", async () => {
    ...
    await expect(hardDeleteById(client as never, "User", "ghost")).rejects.toThrow("not found or not soft-deleted");
  });
});
